...
class OverlayService : Service() {
    ...
    private fun updateOverlay() {
        ...
        overlayView.findViewById<TextView>(R.id.overlayBatteryValue).text = when (batteryState) {
            is FreshnessDisplayHelper.DisplayState.Fresh -> "${batteryState.value}%"
            else -> "N/A"
        }
        overlayView.findViewById<TextView>(R.id.overlayPingValue).text = when (pingState) {
            is FreshnessDisplayHelper.DisplayState.Fresh -> "${pingState.value}ms"
            else -> "N/A"
        }
        overlayView.findViewById<TextView>(R.id.overlayTemperatureValue).text = when (tempState) {
            is FreshnessDisplayHelper.DisplayState.Fresh -> "${tempState.value}°C"
            else -> "N/A"
        }
        overlayView.findViewById<TextView>(R.id.overlayFpsValue).text = "N/A"
    }

    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "GameMonitor Overlay", NotificationManager.IMPORTANCE_LOW)
